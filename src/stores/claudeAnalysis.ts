...
import { claudeAnalysisService, type BuildRecommendation } from '@/services/claudeAnalysis'
...
export const useClaudeAnalysisStore = defineStore('claudeAnalysis', () => {
  ...
  const cache = ref<Map<number, BuildRecommendation>>(new Map())
  ...
  async function generateBuildSuggestion(hero: Hero, forceRefresh = false): Promise<void> {
    ...
    try {
      ...
      // Cache the result
      cache.value.set(hero.heroId, result)
    } catch (e) {
